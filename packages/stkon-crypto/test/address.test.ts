...
import { getAddress, StkonAddress } from '../src/address';
...
describe('test address', () => {
  it('should test HamonyAddress', () => {
    const hAddress = new StkonAddress(bytes);
    ...
    expect(StkonAddress.isValidBasic(bytes)).toEqual(true);
    expect(StkonAddress.isValidChecksum(checksum)).toEqual(true);
    expect(StkonAddress.isValidBech32(bech32)).toEqual(true);
    expect(StkonAddress.isValidBech32TestNet(bech32Testnet)).toEqual(true);
