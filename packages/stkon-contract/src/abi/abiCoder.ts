...
import { hexToBN, defineReadOnly } from '@stkon-js/utils';
...
function parseSignatureFunction(fragment: string): FunctionFragment {
  ...
  if (abi.name === 'constructor') {
    ...
    if (abi.outputs.length) {
      delete (abi as any).name;
      delete (abi as any).outputs;
    }
  }
...
class CoderBoolean extends Coder {
  ...
  decode(data: Uint8Array, offset: number): DecodedResult {
    ...
    try {
      ...
      if ((error as { reason: string }).reason === 'insufficient data for uint256 type') {
        throwError('insufficient data for boolean type', INVALID_ARGUMENT, {
          arg: this.localName,
          coderType: 'boolean',
          value: (error as { value: any }).value,
        });
      }
      throw error;
    }
  ...
  encode(value: Arrayish): Uint8Array {
    ...
    try {
      ...
      throwError('invalid ' + this.name + ' value', INVALID_ARGUMENT, {
        arg: this.localName,
        coderType: this.name,
        value: (error as { value: any }).value || value,
      });
    }
    return result;
  }
