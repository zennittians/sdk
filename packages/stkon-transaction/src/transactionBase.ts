...
import { BN, getAddress, StkonAddress } from '@stkon-js/crypto';
import { hexToNumber } from '@stkon-js/utils';
import { Messenger, RPCMethod, Emitter, HttpProvider, NewHeaders } from '@stkon-js/network';
...
export class TransactionBase implements AbstractTransaction {
  static normalizeAddress(address: string) {
    if (address === '0x') {
      ...
      StkonAddress.isValidChecksum(address) ||
      StkonAddress.isValidBech32(address) ||
      StkonAddress.isValidBech32TestNet(address)
    ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      newHeads.then((p) => {
        p.onData(async (data: any) => {
          const blockNumber =
            this.messenger.chainPrefix === 'stk'
