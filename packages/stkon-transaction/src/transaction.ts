...
import {
  ...
  Signature,
  splitSignature,
} from '@stkon-js/crypto';
import { add0xToString, numberToHex, ChainType, Unit } from '@stkon-js/utils';
import { Messenger, RPCMethod } from '@stkon-js/network';
...
class Transaction extends TransactionBase {
  ...
  getRLPUnsigned(): [string, any[]] {
    ...
    const fields =
      this.messenger.chainType === ChainType.Stkon ? transactionFields : transactionFieldsETH;
  ...
  getRLPSigned(raw: any[], signature: Signature): string {
    // temp setting to be compatible with eth
    const rawLength = this.messenger.chainType === ChainType.Stkon ? 11 : 9;
  ...
  recover(rawTransaction: string): Transaction {
    // temp setting to be compatible with eth
    const recovered =
      this.messenger.chainType === ChainType.Stkon
