...
import { isAddress, ChainType, hexToBN, ChainID, isValidAddress } from '@stkon-js/utils';
import { getAddressFromPrivateKey, getAddress } from '@stkon-js/crypto';
...
describe('test sign tranction', () => {
  ...
  it('should test sign transaction with Stkon settings', () => {
    const stkMessenger = new Messenger(provider, ChainType.Stkon, ChainID.Default);
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < reTxnVectors.length; i += 1) {
      ...
      transaction.setMessenger(stkMessenger);
  ...
  it('should test recover from StkonSignedTransaction', () => {
    const stkMessenger = new Messenger(provider, ChainType.Stkon, ChainID.Default);
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < reTxnVectors.length; i += 1) {
      const vector = reTxnVectors[i];
      const factory = new TransactionFactory(stkMessenger);
      factory.setMessenger(stkMessenger);
