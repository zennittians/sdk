...
import { ChainID, ChainType, Unit, isBech32Address } from '@stkon-js/utils';
import { HDNode } from '@stkon-js/account';
import { fromBech32, HRP } from '@stkon-js/crypto';
...
export class TruffleProvider extends HDNode {
  ...
  async send(...args: [RPCRequestPayload<any>, any]) {
    ...
    switch (newMethod) {
      ...
      case 'stk_getTransactionReceipt': {
        const result = await this.provider.send(
          {
            id,
      ...
      case 'web3_clientVersion': {
        callback(null, {
          result: `Stkon/${packageInfo.version}/@stkon-js`,
          id,
          jsonrpc: '2.0',
        });
        return {
