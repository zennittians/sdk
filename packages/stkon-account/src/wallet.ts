...
import { bip39, hdkey, EncryptOptions, getAddress, generatePrivateKey } from '@stkon-js/crypto';
import { Messenger } from '@stkon-js/network';
import { isPrivateKey, isAddress, ChainType } from '@stkon-js/utils';
...
class Wallet {
  ...
  addByMnemonic(phrase: string, index: number = 0) {
    ...
    const path = this.messenger.chainType === ChainType.Stkon ? '1023' : '60';
