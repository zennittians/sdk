...
import { bip39, hdkey, getAddress, BN, Signature } from '@stkon-js/crypto';
import {
  HDPath,
  // defineReadOnly,
  isHttp,
...
import { Messenger, HttpProvider, WSProvider, RPCMethod } from '@stkon-js/network';
import { Transaction, TxStatus, TransasctionReceipt } from '@stkon-js/transaction';
...
export class HDNode {
  ...
  constructor(
    ...
    shardID: number = 0,
    chainType: ChainType = ChainType.Stkon,
    ...
    this.addresses = [];
    this.wallets = {};
    this.path = chainType === ChainType.Stkon ? HDPath : `m/44'/60'/0'/0/`;
    this.index = index;
  ...
  getHdWallet(mnemonic: string) {
    ...
    for (let i = this.index; i < this.index + this.addressCount; i++) {
      ...
      const prv: string = privateKey.toString('hex');
      const account: Account = new Account(prv);
