...
var HDNode = /** @class */ (function () {
    function HDNode(provider, menmonic, index, addressCount, shardID, chainType, chainId, gasLimit, gasPrice) {
        ...
        if (chainType === void 0) { chainType = utils_1.ChainType.Stkon; }
        ...
        this.addresses = [];
        this.wallets = {};
        this.path = chainType === utils_1.ChainType.Stkon ? utils_1.HDPath : "m/44'/60'/0'/0/";
        this.index = index;
    ...
    HDNode.prototype.setProvider = function (provider) {
        if ((0, utils_1.isHttp)(provider) && typeof provider === 'string') {
            return new network_1.HttpProvider(provider);
        }
        else if (provider instanceof network_1.HttpProvider) {
            return provider;
        }
        else if ((0, utils_1.isWs)(provider) && typeof provider === 'string') {
    ...
    HDNode.prototype.getHdWallet = function (mnemonic) {
        ...
        for (var i = this.index; i < this.index + this.addressCount; i++) {
            ...
            var childKey = this.hdwallet.derive("".concat(this.path).concat(i));
            var privateKey = childKey.privateKey;
            var prv = privateKey.toString('hex');
    ...
    HDNode.prototype.signTransaction = function (txParams) {
        return tslib_1.__awaiter(this, void 0, void 0, function () {
            ...
            return tslib_1.__generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        from = txParams.from ? (0, crypto_1.getAddress)(txParams.from).checksum : '0x';
                        return [4 /*yield*/, this.messenger.send(network_1.RPCMethod.GetAccountNonce, [from, 'latest'], 'stk', this.shardID)];
                    case 1:
                        accountNonce = _a.sent();
                        to = txParams.to ? (0, crypto_1.getAddress)(txParams.to).checksum : '0x';
                        gasLimit = new utils_1.Unit('0').asWei().toWei();
                        if (txParams.gas !== undefined && (0, utils_1.isHex)(txParams.gas)) {
                        ...
                        if (txParams.gasLimit !== undefined && (0, utils_1.isHex)(txParams.gasLimit)) {
                        ...
                        if (txParams.gasPrice !== undefined && (0, utils_1.isHex)(txParams.gasPrice)) {
                        ...
                        value = txParams.value !== undefined && (0, utils_1.isHex)(txParams.value) ? txParams.value : '0';
                        nonce = txParams.nonce !== undefined && (0, utils_1.isHex)(txParams.nonce)
                            ? Number.parseInt((0, utils_1.hexToNumber)(txParams.nonce), 10)
                            : accountNonce.result;
                        data = txParams.data !== undefined && (0, utils_1.isHex)(txParams.data) ? txParams.data : '0x';
                        prv = this.wallets[from].privateKey;
                        signerAccount = new account_1.Account(prv, this.messenger);
                        tx = new transaction_1.Transaction(tslib_1.__assign(tslib_1.__assign({}, txParams), { from: from, to: to, gasLimit: gasLimit, gasPrice: gasPrice, value: value, nonce: nonce, data: data, shardID: this.shardID, chainId: this.messenger.chainId }), this.messenger, transaction_1.TxStatus.INTIALIZED);
