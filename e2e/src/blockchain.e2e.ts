...
describe('e2e test blockchain', () => {
  ...
  it('should test net_peerCount', async () => {
    ...
    expect(stkon.utils.isHex(peerCount.result)).toEqual(true);
  });
  ...
  it('should test stk_getBlockByNumber', async () => {
    ...
    expect(res.responseType).toEqual('raw');
    expect(stkon.utils.isHex(size)).toEqual(true);
    ...
    expect(stkon.utils.isHex(res2.result.size)).toEqual(true);
  ...
  it('should test stk_getBlockByHash', async () => {
    fetch.mockResponse(
      JSON.stringify({
        "jsonrpc": "2.0",
        "id": 1,
    ...
    expect(stkon.utils.isHex(res.result.size)).toEqual(true);
    expect(checkBlockData(res.result)).toEqual(true);
  ...
  // account related
  it('should test stk_getBalance', async () => {
    fetch.mockResponseOnce(
      JSON.stringify({"jsonrpc": "2.0", "id": 1, "result": "0x10"}),
    );
    const balance = await bc.getBalance({ address: testAccount.Address });
    expect(stkon.utils.isHex(balance.result)).toEqual(true);
...
function checkBlockData(data: any) {
  return stkon.utils.validateArgs(
    data,
    {
      difficulty: [stkon.utils.isNumber],
      // tslint:disable-next-line: no-shadowed-variable
      extraData: [(data: any) => data === '0x' || stkon.utils.isHex(data)],
      gasLimit: [stkon.utils.isHex],
      gasUsed: [stkon.utils.isHex],
      hash: [stkon.utils.isHash],
      logsBloom: [stkon.utils.isHex],
      miner: [stkon.utils.isBech32Address],
      mixHash: [stkon.utils.isHash],
      nonce: [stkon.utils.isNumber],
      number: [stkon.utils.isHex],
      parentHash: [stkon.utils.isHash],
      receiptsRoot: [stkon.utils.isHash],
      size: [stkon.utils.isHex],
      stateRoot: [stkon.utils.isHash],
      timestamp: [stkon.utils.isHex],
      transactionsRoot: [stkon.utils.isHash],
      uncles: [stkon.utils.isArray],
    },
    { transactions: [stkon.utils.isArray] },
  );
}